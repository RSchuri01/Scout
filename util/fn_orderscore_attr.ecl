...
EXPORT fn_orderscore_attr(dataset(layout.base_transaction) infile) := MODULE

mbs_logs_slim := infile(std.str.touppercase(TRIM(esp_method)) = 'ORDERSCORE' and regexfind('<Attributes>', outputxml));
...
add_product := project(parsedoutput, transform(layout.attributes,
						self.esp_method := 'OrderScore', self := left));

export generate := OUTPUT(add_product,,'~thor::scout::score_logs::orderscore_parsed_attributes', thor, overwrite, compressed);

export getDS := dataset('~thor::scout::score_logs::orderscore_parsed_attributes', layout.attributes, thor);
