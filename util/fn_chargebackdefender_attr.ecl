...
EXPORT fn_chargebackdefender_attr(dataset(layout.base_transaction) infile) := MODULE

mbs_logs_slim := infile(std.str.touppercase(TRIM(esp_method)) = 'CHARGEBACKDEFENDER' and regexfind('<Attributes>', outputxml));
...
export generate := OUTPUT(add_product,,'~thor::scout::score_logs::ChargebackDefender_parsed_attributes', thor, overwrite, compressed);

export getDS := dataset('~thor::scout::score_logs::ChargebackDefender_parsed_attributes', layout.attributes, thor);
