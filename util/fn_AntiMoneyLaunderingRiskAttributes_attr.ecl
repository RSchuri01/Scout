...
EXPORT fn_AntiMoneyLaunderingRiskAttributes_attr(dataset(layout.base_transaction) infile) := MODULE

mbs_logs_slim := infile(std.str.touppercase(TRIM(esp_method)) in ['ANTIMONEYLAUNDERINGRISKATTRIBU', 'ANTIMONEYLAUNDERINGRISKATTRIBUTES'] and regexfind('<Attributes>', outputxml));
...
export generate := OUTPUT(projectedData,,'~thor::scout::score_logs::AntiMoneyLaunderingRiskAttributes_parsed_attributes', thor, overwrite, compressed);

export getDS := dataset('~thor::scout::score_logs::AntiMoneyLaunderingRiskAttributes_parsed_attributes', layout.attributes, thor);
