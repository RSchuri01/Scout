#workunit('name', 'VerificationOfOccupancy_Pull_SCOUT_Logs');
#STORED('historyfreq', '7yrs');
IMPORT scout, STD;

Product := 'VerificationOfOccupancy';
...
outputFile := '~fallen::out::VOO_SCOUT_' + BeginDate + '-' + EndDate + '_' + AccountIDs[1];
...
ScoutFile_Raw := IF(AccountIDs[1] != '', Pulled_ScoutFile(STD.Str.ToUpperCase(TRIM(esp_method)) IN ['VERIFICATIONOFOCCUPANCY'] AND datetime[1..8] BETWEEN BeginDate AND EndDate AND (string)company_id IN AccountIDs AND company_id NOT IN scout.ad_hoc_reporting.constants.IgnoredAccountIDs),
									     Pulled_ScoutFile(STD.Str.ToUpperCase(TRIM(esp_method)) IN ['VERIFICATIONOFOCCUPANCY'] AND datetime[1..8] BETWEEN BeginDate AND EndDate AND STD.Str.ToLowerCase(TRIM(login_id)) NOT IN scout.ad_hoc_reporting.constants.IgnoredLogins AND company_id NOT IN scout.ad_hoc_reporting.constants.IgnoredAccountIDs));
...
parsedInput := PARSE(Good_Logs, inputxml, parseInput(), XML('VerificationOfOccupancy'));
...
Parsed_Layout parseOutput () := TRANSFORM
	...
	SELF := [];
END;
parsedOutput := PARSE(Good_Logs, outputxml, parseOutput(), XML('VerificationOfOccupancy'));
