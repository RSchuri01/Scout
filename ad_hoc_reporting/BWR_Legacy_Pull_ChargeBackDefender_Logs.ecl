#workunit('name', 'BWR_Pull_ChargeBackDefender_SCOUT_Logs');
#STORED('historyfreq', '7yrs');
import scout, STD;

Product := 'ChargebackDefender';
...
outputFile := '~fallen::out::ChargebackDefender_SCOUT_' + BeginDate + '-' + EndDate + '_' + AccountIDs[1];
...
ScoutFile_Raw := IF(AccountIDs[1] != '', Pulled_ScoutFile(STD.Str.ToUpperCase(TRIM(esp_method)) IN ['CHARGEBACKDEFENDER'] AND datetime[1..8] BETWEEN BeginDate AND EndDate AND (string)company_id IN AccountIDs AND company_id NOT IN scout.ad_hoc_reporting.constants.IgnoredAccountIDs),
									     Pulled_ScoutFile(STD.Str.ToUpperCase(TRIM(esp_method)) IN ['CHARGEBACKDEFENDER'] AND datetime[1..8] BETWEEN BeginDate AND EndDate AND STD.Str.ToLowerCase(TRIM(login_id)) NOT IN scout.ad_hoc_reporting.constants.IgnoredLogins AND company_id NOT IN scout.ad_hoc_reporting.constants.IgnoredAccountIDs));
...
scout.ad_hoc_reporting.Layouts.Parsed_ChargeBackDefender_Layout parseInput () := TRANSFORM
...
//BillTo
	...
	SELF.SSN						:= scout.ad_hoc_reporting.Common.ParseSSN(XMLTEXT('SearchBy/BillTo/SSN'));
	SELF.DOB						:= TRIM(XMLTEXT('SearchBy/BillTo/DOB')) + 
												scout.ad_hoc_reporting.Common.ParseDate(XMLTEXT('SearchBy/BillTo/DOB/Year'), 
												XMLTEXT('SearchBy/BillTo/DOB/Month'), XMLTEXT('SearchBy/BillTo/DOB/Day'));
	SELF.Address				:= scout.ad_hoc_reporting.Common.ParseAddress(XMLTEXT('SearchBy/BillTo/Address/StreetAddress1'), 
	...
	SELF.Zip						:= scout.ad_hoc_reporting.Common.ParseZIP(XMLTEXT('SearchBy/BillTo/Address/Zip5'));
	SELF.DL							:= TRIM(XMLTEXT('SearchBy/BillTo/DriverLicenseNumber'));
	SELF.DLState				:= TRIM(XMLTEXT('SearchBy/BillTo/DriverLicenseState'));

	SELF.HomePhone			:= scout.ad_hoc_reporting.Common.ParsePhone(XMLTEXT('SearchBy/BillTo/Phone10'));
	...
	SELF.DOB2						:= TRIM(XMLTEXT('SearchBy/ShipTo/DOB')) + 
												scout.ad_hoc_reporting.Common.ParseDate(XMLTEXT('SearchBy/ShipTo/DOB/Year'), 
												XMLTEXT('SearchBy/ShipTo/DOB/Month'), XMLTEXT('SearchBy/ShipTo/DOB/Day'));
	SELF.Address2				:= scout.ad_hoc_reporting.Common.ParseAddress(XMLTEXT('SearchBy/ShipTo/Address/StreetAddress1'), 
	...
	SELF.Zip2						:= scout.ad_hoc_reporting.Common.ParseZIP(XMLTEXT('SearchBy/ShipTo/Address/Zip5'));
	SELF.DL2						:= TRIM(XMLTEXT('SearchBy/ShipTo/DriverLicenseNumber'));
	SELF.DLState2				:= TRIM(XMLTEXT('SearchBy/ShipTo/DriverLicenseState'));
	SELF.HomePhone2			:= scout.ad_hoc_reporting.Common.ParsePhone(XMLTEXT('SearchBy/ShipTo/Phone10'));
	...
	SELF := [];
END;
parsedInput := DISTRIBUTE(PARSE(Good_Logs, inputxml, parseInput(), XML('ChargebackDefender')), HASH64(TransactionID));
OUTPUT(CHOOSEN(parsedInput, eyeball), NAMED('Sample_Parsed_Input'));

scout.ad_hoc_reporting.Layouts.Parsed_ChargeBackDefender_Layout parseOutput () := TRANSFORM
...
//extras	
	...
	SELF.areacodesplitdate	:= TRIM(XMLTEXT('Result/BillTo/NewAreaCode/EffectiveDate')) + 
												scout.ad_hoc_reporting.Common.ParseDate(XMLTEXT('Result/BillTo/NewAreaCode/EffectiveDate/Year'), 
...
//extras
	...
	SELF.areacodesplitdate2	:= TRIM(XMLTEXT('Result/ShipTo/NewAreaCode/EffectiveDate')) + 
												scout.ad_hoc_reporting.Common.ParseDate(XMLTEXT('Result/ShipTo/NewAreaCode/EffectiveDate/Year'), 
...
parsedOutput := PARSE(Good_Logs, outputxml, parseOutput(), XML('ChargebackDefender'));
OUTPUT(CHOOSEN(parsedOutput, eyeball), NAMED('Sample_Parsed_Output'));
OUTPUT(count(parsedOutput), NAMED('parsedOutput_count'));
  
 scout.ad_hoc_reporting.Layouts.Parsed_ChargeBackDefender_Layout combineParsedRecords(scout.ad_hoc_reporting.Layouts.Parsed_ChargeBackDefender_Layout le, scout.ad_hoc_reporting.Layouts.Parsed_ChargeBackDefender_Layout ri) := TRANSFORM
