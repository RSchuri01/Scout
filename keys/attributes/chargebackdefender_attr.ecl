...
EXPORT chargebackdefender_attr := MODULE

SHARED infile := scout.logs.files_stg.online_stg_ds : INDEPENDENT;


espName := 'CHARGEBACKDEFENDER';
...
mbs_logs := project(mbs_base_slim, transform(layout.base_transaction, 
			SELF.outputxml := '<ChargebackDefender><Datetime>' + LEFT.Datetime + '</Datetime>' + LEFT.outputxml + '</ChargebackDefender>', self := left, self := []));
...
parsedoutput:= PARSE(mbs_logs, outputxml, parseOutput(), XML('ChargebackDefender'));


SHARED projectedData := project(parsedoutput, transform(layout.attributes,
					   self.esp_method := 'ChargebackDefender', self := left));

SHARED ChargebackDefender := dedup(
	        normalize(projectedData, 2160, scout.logs.util.fn_transformattributesByNormalize(left, counter), local), all, local);

export idxKeyName := scout.common.constants.chargebackdefender_attr_keyName;

export twoyrHistoryFileName := scout.logs.keys.key_constants('2yearhistory').ChargebackDefender_idx;

SHARED subIdxFileName(String pversion) := scout.logs.util.fn_getMySubKeyNameByKey(idxKeyName, pversion);

SHARED idxLayout := RECORDOF(ChargebackDefender);
...
export indexDailyStgFile(String pv, boolean isRollupAsked)  := scout.logs.util.fn_buildSubKeyAndAddToSuperKey(INDEX(ChargebackDefender(transaction_id <> '' and AttributeName <> ''), 
                                 {ChargebackDefender.transaction_id, ChargebackDefender.datetime}, {ChargebackDefender},
